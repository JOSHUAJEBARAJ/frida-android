...
lbl43: // 1 sources:
    ...
    Drawable getDrawableFromResourceUri(Uri uri) throws FileNotFoundException {
        ...
        if (!TextUtils.isEmpty((CharSequence)charSequence)) {
            Resources resources;
            try {
                resources = this.mContext.getPackageManager().getResourcesForApplication(charSequence);
            }
            catch (PackageManager.NameNotFoundException nameNotFoundException) {
                StringBuilder stringBuilder = new StringBuilder();
            ...
            if (list != null) {
                ...
                if (n == 2) {
                    n = resources.getIdentifier((String)list.get(1), (String)list.get(0), charSequence);
                    if (n != 0) {
