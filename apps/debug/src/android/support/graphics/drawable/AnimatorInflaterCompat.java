...
public class AnimatorInflaterCompat {
     ...
     */
    private static Animator createAnimatorFromXml(Context object, Resources iterator, Resources.Theme theme, XmlPullParser xmlPullParser, AttributeSet attributeSet, AnimatorSet animatorSet, int n, float f) throws XmlPullParserException, IOException {
        int n2;
        ...
        while (((n2 = xmlPullParser.next()) != 3 || xmlPullParser.getDepth() > n3) && n2 != 1) {
            if (n2 != 2) continue;
            PropertyValuesHolder[] arrpropertyValuesHolder = xmlPullParser.getName();
            n2 = 0;
            if (arrpropertyValuesHolder.equals("objectAnimator")) {
                objectAnimator = AnimatorInflaterCompat.loadObjectAnimator((Context)object, iterator, theme, attributeSet, f, xmlPullParser);
            } else if (arrpropertyValuesHolder.equals("animator")) {
                objectAnimator = AnimatorInflaterCompat.loadAnimator((Context)object, iterator, theme, attributeSet, null, f, xmlPullParser);
            } else if (arrpropertyValuesHolder.equals("set")) {
                objectAnimator = new AnimatorSet();
                arrpropertyValuesHolder = TypedArrayUtils.obtainAttributes(iterator, theme, attributeSet, AndroidResources.STYLEABLE_ANIMATOR_SET);
                int n4 = TypedArrayUtils.getNamedInt((TypedArray)arrpropertyValuesHolder, xmlPullParser, "ordering", 0, 0);
                AnimatorInflaterCompat.createAnimatorFromXml((Context)object, iterator, theme, xmlPullParser, attributeSet, (AnimatorSet)objectAnimator, n4, f);
                arrpropertyValuesHolder.recycle();
            } else {
                if (!arrpropertyValuesHolder.equals("propertyValuesHolder")) {
                    object = new StringBuilder();
                    object.append("Unknown animator name: ");
                    object.append(xmlPullParser.getName());
                    throw new RuntimeException(object.toString());
                }
                arrpropertyValuesHolder = AnimatorInflaterCompat.loadValues((Context)object, iterator, theme, xmlPullParser, Xml.asAttributeSet((XmlPullParser)xmlPullParser));
            ...
            arrpropertyValuesHolder = object2;
            if (animatorSet != null) {
                arrpropertyValuesHolder = object2;
                if (n2 == 0) {
                    arrpropertyValuesHolder = object2;
            ...
            object2 = arrpropertyValuesHolder;
        }
        if (animatorSet != null && object2 != null) {
            object = new Animator[object2.size()];
            n2 = 0;
            iterator = object2.iterator();
            while (iterator.hasNext()) {
                object[n2] = (Animator)iterator.next();
                ++n2;
            }
            if (n == 0) {
    ...
    private static void setupPathMotion(Path path, ObjectAnimator objectAnimator, float f, String object, String string2) {
        ...
        int n2 = 0;
        for (int i = 0; i < n; ++i) {
            path.getPosTan(f - ((Float)arrayList.get(n2)).floatValue(), arrf3, null);
            arrf2[i] = arrf3[0];
            arrf[i] = arrf3[1];
            f += f2;
            int n3 = n2;
