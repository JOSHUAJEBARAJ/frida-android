...
implements Animatable {
        ...
        Animation animation = new Animation(){
            ...
            public void applyTransformation(float f, Transformation transformation) {
                ...
                if (f <= 0.5f) {
                    ...
                    ring.setStartTrim(f4 + (0.8f - f2) * f6);
                }
                if (f > 0.5f) {
                    f4 = (f - 0.5f) / 0.5f;
                    f4 = MATERIAL_INTERPOLATOR.getInterpolation(f4);
                    ring.setEndTrim(f3 + f4 * (0.8f - f2));
                }
                ring.setRotation(f5 + 0.25f * f);
                f2 = MaterialProgressDrawable.this.mRotationCount / 5.0f;
                MaterialProgressDrawable.this.setRotation(216.0f * f + 1080.0f * f2);
            }
        };
    ...
    private static class Ring {
        ...
        public void setInsets(int n, int n2) {
            ...
            f = this.mRingCenterRadius <= 0.0 || f < 0.0f ? (float)Math.ceil(this.mStrokeWidth / 2.0f) : (float)((double)(f / 2.0f) - this.mRingCenterRadius);
            this.mStrokeInset = f;
        }
